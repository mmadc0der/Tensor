...
template <typename T>
Tensor<T> make_scalar(const T& value) {
    ...
    DTensor dt{
        ...
        []{
            ...
            else if constexpr (std::is_same_v<T, float>) return DType::f32;
            else if constexpr (std::is_same_v<T, double>) return DType::f64;
            else return DType::f32;
        }(),
        Layout::contiguous,
        /*is_contiguous*/true,
        /*requires_grad*/false
...
template <typename T>
inline Tensor<T> ones(const std::vector<int64_t>& shape) {
    Tensor<T> t = empty<T>(shape);
    T* p = t.data();
    for (int64_t i = 0, n = t.numel(); i < n; ++i) p[i] = T{1};
    return t;
}
...
// View ops
inline DTensor reshape(const DTensor& t, const std::vector<int64_t>& new_shape) {
    ...
    assert(is_contiguous(t) && "reshape: only contiguous tensors supported for now");
    auto ns = default_strides(new_shape);
    DTensor out{ t.storage(), new_shape, std::move(ns), t.offset(), t.dtype(), t.layout(), true, t.requires_grad() };
...
inline DTensor permute(const DTensor& t, const std::vector<int>& perm) {
    ...
    DTensor out{ t.storage(), std::move(new_shape), std::move(new_stride), t.offset(), t.dtype(), t.layout(), false, t.requires_grad() };
    out.set_grad_fn(t.grad_fn());
    return out;
}
