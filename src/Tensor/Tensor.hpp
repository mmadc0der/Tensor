...
enum class DType : uint8_t { f16, bf16, f32, f64, i32, i64 };

// Layout tag for fast-paths and heuristics
enum class Layout : uint8_t { contiguous, channels_last, blocked };
...
public:
  ...
  void *data() noexcept {
    ...
    return static_cast<void *>(
        base + offset_ * static_cast<int64_t>(dtype_size(dtype_)));
  }
  ...
  void set_grad_fn(std::weak_ptr<AutogradNode> fn) noexcept {
...
public:
  Tensor() {
    ...
    dt_ = DTensor(std::move(st), std::move(shp), std::move(str),
                  /*offset*/ 0, dt, Layout::contiguous,
                  /*is_contiguous*/ true,
                  /*requires_grad*/ false);
  }
  ...
  void set_grad(std::shared_ptr<DTensor> g) noexcept {
    dt_.set_grad(std::move(g));
  }
